...
export class CreateInvoiceItemsTable1782491740625 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE "invoice_items" ("id" uuid NOT NULL DEFAULT gen_random_uuid(), "invoice_id" uuid NOT NULL, "name" character varying(255) NOT NULL, "quantity" integer NOT NULL, "rate" numeric(14,2) NOT NULL, CONSTRAINT "chk_invoice_items_rate_positive" CHECK ("rate" > 0), CONSTRAINT "chk_invoice_items_quantity_positive" CHECK ("quantity" > 0), CONSTRAINT "pk_invoice_items" PRIMARY KEY ("id"))`);
